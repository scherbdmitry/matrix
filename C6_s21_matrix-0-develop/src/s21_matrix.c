...
int s21_sum_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
  ...
  if (A->columns != B->columns || A->rows != B->rows) return CALC_MATRIX_ERROR;
...
int s21_mult_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
  if (!A || !B || !result || !A->matrix || !B->matrix) return INCORRECT_MATRIX;
  if (A->columns != B->rows) return CALC_MATRIX_ERROR;
...
int s21_calc_complements(matrix_t *A, matrix_t *result) {
  ...
  if (A->rows != A->columns) return CALC_MATRIX_ERROR;
  int size = A->rows;
    ...
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        ...
        if (!s21_create_matrix(minor_size, minor_size, &m_minor)) {
          ...
          result->matrix[i][j] = s21_searchDeterminant(&m_minor, minor_size) *
                                 pow(-1, (i + 1 + j + 1));
        }
  ...
  if (size == 1) {
    ...
    for (int k = 0; k < size; k++) {
      ...
      determinant += pow(-1, k + 2) * A->matrix[0][k] *
                     s21_searchDeterminant(&minor, size - 1);
...
int s21_determinant(matrix_t *A, double *result) {
  if (!A || !result || !A->matrix) return INCORRECT_MATRIX;
  if (A->rows != A->columns) return CALC_MATRIX_ERROR;
  *result = s21_searchDeterminant(A, A->rows);
  return OK;
}
